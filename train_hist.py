...
class Trainer:
    ...
    def optimize(self, train_loader, hidden_pres, is_train=True):
        ...
        for idx, sample in enumerate(train_loader):
            ...
            # b*2, b*2, 1*b*256, b*1
            actions_prob, action_logprob, hidden_pres, value = self.model.forward(imgs=imgs,
                                                                                  action=actions,
            ...
            # ads = rewards - value.detach()
            ads = rewards - value
            action_log_ads = action_logprob * ads
            actions = actions.long()
            actions = actions.squeeze(1)
            # import pdb
            # pdb.set_trace()
            # action_log_ads = torch.LongTensor(action_log_ads)

            rewards = rewards
            value = value

            loss1 = - self.loss1(action_log_ads, actions)
            loss2 = self.loss2(value, rewards)
            loss = loss1+loss2
            if is_train:
                self.optimizer.zero_grad()
                loss.backward()

                torch.nn.utils.clip_grad_norm(self.model.actor.parameters(), 0.5)
                torch.nn.utils.clip_grad_norm(self.model.critic.parameters(), 0.5)
                self.optimizer.step()
